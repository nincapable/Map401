...
void calcul_contour_mult(Image I, Robot *R, liste_contour *L,int *compte){
    ...
    Contour *C1=(Contour*)malloc(sizeof(Contour));
    Image M = masque(I, compte);

    while ((*compte) > 0)
    {
        position_initiale(M,R);
        contour_new(I,&M, R, C1,compte);
        C = concatener_contour(C, C1);
        printf("L->taille :%d\n", L->taille);
        // printf("%d\n", (*compte));
    }
}
...
Contour concatener_contour(Contour C1, Contour C2){
    Contour *C3=(Contour*)malloc(sizeof(Contour));
    C1 = concatener_liste_Point(C1, C2);
    C3 = -2, -2;
    C1 = concatener_liste_Point(C1, C3);
    return C1;
}
